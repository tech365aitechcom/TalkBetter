...
import { Document, FileState } from '../../../../shared/interfaces/document'
...
const DocumentsPage = () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`${BACKEND_BASE_URL}/api/doc/deleteDoc/${id}`)
      // Remove the deleted document from the state
      setDocuments((prevDocs) =>
        prevDocs.filter((doc: Document) => doc._id !== id)
      )
    } catch (error) {
  ...
  const handleConfirmUpload = async () => {
    ...
    // Hide confirmation once the upload is confirmed
    setFileState((prev) => ({ ...prev, showConfirmation: false }))

    try {
      // Here, we use FormData to send the file
      const formData = new FormData()
      formData.append('document', fileState.selectedFile)
      formData.append('userId', 'harsh')
      ...
      <div className='flex h-full w-full justify-between gap-10'>
        ...
        <div className='flex flex-col w-full'>
          ...
          >
            ...
            ) : fileState.error || documents.length === 0 ? (
